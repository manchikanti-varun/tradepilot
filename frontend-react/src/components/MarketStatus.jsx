...
export default function MarketStatus() {
  ...
  const statusColor = countdown.status === 'OPEN' ? 'text-green-400' :
      ...
      <div className="flex items-center gap-2">
        ...
        <span className={`text-[11px] font-bold ${statusColor}`}>
          {countdown.status === 'OPEN' ? 'MARKET OPEN' :
           countdown.status === 'PRE_MARKET' ? 'PRE-MARKET' : 'CLOSED'}
        </span>
        <span className="text-[10px] text-gray-500">{countdown.display}</span>
      </div>
      {expiry?.is_expiry_day && (
        <div className="flex items-center gap-1 bg-amber-500/15 px-2 py-0.5 rounded-full">
          <AlertTriangle size={9} className="text-amber-400" />
          <span className="text-[9px] text-amber-400 font-bold">EXPIRY DAY</span>
        </div>
      )}
    </div>
  )
